...
import numpy as np
from beamz import (
    Design, Material, Rectangle, Simulation, PML, ModeSource,
    LIGHT_SPEED, um, calc_optimal_fdtd_params, ramped_cosine
)
...
class TestModeSourceEffectiveIndex:
    ...
    def test_neff_within_bounds(self):
        ...
        assert neff < n_core + 0.1, f"n_eff={neff:.4f} should not exceed n_core={n_core}"
    ...
    def test_neff_increases_with_core_width(self):
        ...
        assert neffs[1] >= neffs[0] - 0.01, f"n_eff should increase with core width: {neffs}"
        assert neffs[2] >= neffs[1] - 0.01, f"n_eff should increase with core width: {neffs}"
...
class TestModeSourcePropagation:
    ...
    def test_mode_stays_confined_in_waveguide(self, waveguide_domain):
        ...
        wg_region = slice(center_y - 2*core_half_cells, center_y + 2*core_half_cells)
