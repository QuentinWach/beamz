...
def test_3d_mode_source_suppresses_counterpropagating_power_on_both_grids(
    ...
    # A raw rectilinear FDFD seed used to pass this test at roughly 4.7% while
    # radiating enough power to bias crossing insertion loss by about 0.15 dB.
    # Both grids must launch the Yee-refined mode with less than 1% reverse power.
    assert counter_power / forward_power < 0.01


@pytest.mark.compiled
def test_rectilinear_mode_source_launches_unit_power_in_straight_guide():
    """Auto-grid mode launch must not create the crossing's former 0.15 dB bias."""
    wavelength = 1.31e-6
    frequency = LIGHT_SPEED / wavelength
    core_index = 3.67
    clad_index = 1.45
    core_width = 0.35e-6
    core_height = 0.161e-6
    source_x = -1.25e-6
    plane_size = (0.0, 1.4e-6, 0.644e-6)
    reference_dx = wavelength / (10 * core_index)

    design = Design(background=Material(permittivity=clad_index**2))
    design += Box(
        center=(0.0, 0.0, 0.0),
        size=(np.inf, core_width, core_height),
        material=Material(permittivity=core_index**2),
    )
    source = ModeSource(
        center=(source_x, 0.0, 0.0),
    ...
    simulation = Simulation(
        domain=(4.0e-6, 3.0e-6, 1.965e-6),
        grid_spec=GridSpec.auto(
            wavelength=wavelength,
            min_steps_per_wvl=10,
            max_scale=1.2,
            snapping_points=((0.0, 0.0, 0.0),),
        ),
        design=design,
        sources=[source],
        monitors=monitors,
    ...
    assert simulation.grid.metric_kind == "rectilinear"
    assert result.launched_power() == pytest.approx(source.power, rel=1e-6)
    assert forward_power == pytest.approx(source.power, rel=0.02)
    assert counter_power / forward_power < 1e-3
