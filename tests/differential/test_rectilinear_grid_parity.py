"""Behavioral parity checks against pinned rectilinear-grid references."""
...
import beamz as bz
from tests.differential.rectilinear_grid_cases import (
    REFERENCE_SCHEMA,
    beamz_grid,
...
REFERENCE_PATH = Path(__file__).with_name("rectilinear_grid_references.json")
REFERENCE = json.loads(REFERENCE_PATH.read_text(encoding="utf-8"))
CASES = parity_cases()
CELL_COUNT_PARITY = Tolerance(
    name="rectilinear_grid_cell_count",
    absolute=0.0,
    relative=0.04,
    rationale=(
...
def test_reference_manifest_is_pinned_and_complete():
    assert REFERENCE["schema"] == REFERENCE_SCHEMA
    assert REFERENCE["reference_revision"] == "v1"
...
def test_automatic_grid_has_reference_cell_count_and_spacing_profile(
    case, axis, validation_metrics
):
    beamz_edges = (
        np.asarray(getattr(beamz_grid(case), f"{axis}_edges"), dtype=np.float64) / bz.um
    )
    reference_edges = _reference_edges(case.name, axis)

    np.testing.assert_allclose(
        beamz_edges[[0, -1]], reference_edges[[0, -1]], rtol=0.0, atol=1e-12
    )
    validation_metrics.check(
        f"{axis}-axis cell count",
        beamz_edges.size - 1,
        reference_edges.size - 1,
        tolerance=CELL_COUNT_PARITY,
        unit="cells",
        backend="beamz-vs-pinned-rectilinear-reference",
    )

    beamz_profile = normalized_spacing_profile(beamz_edges)
    reference_profile = normalized_spacing_profile(reference_edges)
    profile_error = float(
        np.mean(np.abs(beamz_profile - reference_profile)) / np.mean(reference_profile)
    )
    ...
    assert _max_adjacent_ratio(reference_edges) <= case.max_scale * (1.0 + 1e-12)


def test_explicit_snapping_points_are_grid_edges_in_both_datasets():
    ...
    for point in case.snapping_points:
        for axis_index, axis in enumerate("xy"):
            ...
            assert np.any(np.isclose(reference_edges, coordinate, rtol=0.0, atol=1e-12))
