...
make_source = lambda direction: ModeSource(design=design, position=(2.5*µm, H/2), width=WG*4, wavelength=WL, signal=signal, direction=direction, mode_solver="analytical", num_modes=1)
make_monitor = lambda: Monitor(design=design, start=(W/2-WG*2, H-2.5*µm), end=(W/2+WG*2, H-2.5*µm), objective_function=objective, name="out")
...
for step in range(1, STEPS+1):
    ...
    fres = FDTD(design=grid, devices=[make_source("+x"), make_monitor()], time=t).run(live=False, save_memory_mode=True, accumulate_power=True, save_fields=["Ez"], fields_to_cache=["Ez"])
    forward_fields = list(fres.get("Ez", []))
    adj = FDTD(design=grid, devices=[make_source("-y")], time=t); adj.initialize_simulation(save=False, live=False, accumulate_power=False, save_memory_mode=True, fields_to_cache=None)
    grad = np.zeros_like(density)
    for _ in range(adj.num_steps):
        if not forward_fields or not adj.step(): break
        grad += np.real(adj.backend.to_numpy(adj.Ez) * np.conj(forward_fields.pop()))
...
eps = base.copy(); eps[slc] = region_mat.permittivity_grid = EPS_CLAD + filtered[slc]*(EPS_CORE-EPS_CLAD)
np.copyto(grid.permittivity, eps)
final = FDTD(design=grid, devices=[make_source("+x"), make_monitor()], time=t).run(live=False, save_memory_mode=True, accumulate_power=True, save_fields=["Ez"], fields_to_cache=None)
