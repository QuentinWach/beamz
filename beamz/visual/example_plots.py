...
def plot_simulation_overview(
    ...
    *,
    ...
    z_idx = int(
        np.clip(
            round(float(z_focus) / max(float(depth), 1e-30) * (eps.shape[0] - 1)),
            0,
            eps.shape[0] - 1,
        )
    )
    ...
    axes[1].imshow(
        eps[:, y_idx, :],
        origin="lower",
        extent=[0, width / µm, 0, depth / µm],
        cmap="viridis",
        aspect="auto",
    )
    for ax, title, ylabel in (
        (axes[0], "XY overview", "y (um)"),
        (axes[1], "XZ overview", "z (um)"),
    ):
        ax.set_title(title)
        ax.set_xlabel("x (um)")
...
def plot_sparameters_db(
    path: Path,
    wavelengths_um: np.ndarray,
    s_matrix: dict[tuple[str, str], np.ndarray],
    *,
    source_port="o1",
    ports=("o1", "o2", "o3", "o4"),
):
    ...
    for port in ports:
        y_db = 20.0 * np.log10(
            np.maximum(
                np.abs(np.asarray(s_matrix[(port, source_port)], dtype=np.complex128)),
                1e-12,
            )
        )
        ax.plot(
            ...
            label=rf"$|S_{{{port[1:]}{source_port[1:]}}}|$",
        )
