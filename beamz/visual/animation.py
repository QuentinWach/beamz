...
def get_twilight_zero_cmap():
    """Return a diverging colormap with black at zero and white at the edges."""
...
def _register_custom_colormaps():
    ...
    try:
        if "twilight_zero" not in plt.colormaps():
            plt.colormaps.register(get_twilight_zero_cmap(), name="twilight_zero")
    except Exception:
        pass
...
def is_jupyter_environment():
    """Return whether the current process is running inside Jupyter."""
    try:
...
def _refresh_live_canvas(fig, pause=0.0, *, force_draw=False):
    canvas = fig.canvas
    (canvas.draw if force_draw else canvas.draw_idle)()
    canvas.flush_events()
...
def animate_manual_field(
    ...
    *,
    ...
    """Create or update a live Matplotlib view for a 2D field array."""
    import matplotlib.pyplot as plt
...
class JupyterAnimator:
    """Handle Jupyter live animation and stored-frame replay."""
    ...
    def __init__(
        ...
        """Initialize live display and optional replay storage."""
        self.cmap = cmap
    ...
    def _display_frame(
        ...
        """Display a single frame in Jupyter."""
    ...
    def get_animation(self, fps=30):
        """Create an HTML animation from stored frames."""
        import matplotlib.pyplot as plt
    ...
    def get_video(self, filename="animation.mp4", fps=30, dpi=150):
        """Create an MP4 video and display it in Jupyter."""
        try:
