...
def setup_plot_2d(monitor, field_component):
    ...
    state = monitor.state
    state.live_fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(12, 5))
    ...
    state.live_plots["field_line"] = ax1.plot([], [], "b-")[0]

    ax2.set_title("Power vs Time")
    ax2.set_xlabel("Time step")
    ax2.set_ylabel("Total power")
    state.live_plots["power_time"] = ax2.plot([], [], "r-")[0]
    plt.tight_layout()
    plt.ion()
    plt.show()
...
def setup_plot_3d(monitor, field_component):
    ...
    state = monitor.state
    state.live_fig, ((ax1, ax2), (ax3, ax4)) = plt.subplots(
        2, 2, figsize=(12, 10)
    )
    ax1.set_title(f"{field_component} magnitude on plane")
    state.live_plots["field_2d"] = ax1.imshow(
...
def update_plot_2d(monitor):
    ...
    state = monitor.state
    if state.live_fig is None or not state.fields["t"]:
        return
    try:
        latest_field = state.fields["Ez"][-1]
        x_pos = range(len(latest_field))
        state.live_plots["field_line"].set_data(x_pos, latest_field)
        state.live_plots["power_time"].set_data(
            range(len(state.power_history)), state.power_history
        )
        for ax in state.live_fig.axes:
            ax.relim()
            ax.autoscale_view()
        state.live_fig.canvas.draw()
        state.live_fig.canvas.flush_events()
    except Exception:
...
def update_plot_3d(monitor):
    ...
    try:
        latest_field = state.fields["Ez"][-1]
        state.live_plots["field_2d"].set_array(latest_field)
        state.live_plots["field_2d"].set_clim(
        ...
        state.live_plots["field_x"].set_data(
            range(latest_field.shape[1]), latest_field[center_y, :]
        )
        state.live_plots["field_y"].set_data(
            range(latest_field.shape[0]), latest_field[:, center_x]
        )

        for ax in state.live_fig.axes[1:]:
            ax.relim()
            ax.autoscale_view()
        state.live_fig.canvas.draw()
        state.live_fig.canvas.flush_events()
