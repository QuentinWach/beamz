...
class _ModeSourceState:
    ...
    _launch_power_normalization_cache: tuple | None = None
...
def _shift_discrete_mode_to_global(discrete_mode, *, origin_zyx, axis, resolution):
    ...
    if is_dataclass(discrete_mode):
        return replace(discrete_mode, **updates)
...
class ModeSource(RuntimeStateProxy):
    ...
    _RUNTIME_ATTRS = {
        ...
        "_launch_power_normalization_cache",
        "_launch_power_scale",
        "_launch_dt",
        "_discrete_mode",
    ...
    def _setup_discrete_3d_mode_from_micromode(
        ...
        *,
        ...
        local_plane = _local_mode_plane_spec(
            ...
            snapped_region=self._snapped_region,
            aperture_pad_cells=aperture_pad_cells,
        )
        transverse_axes = {
            "x": ("z", "y"),
            "y": ("z", "x"),
    ...
    def _reconstructed_3d_launch_phasor_state(self, fields, *, dt):
        ...
        h_full_next = self._advance_incident_h_3d(fields, full_prev, dt_value)
        h_target_next = self._mask_incident_3d_state_to_launched_side(h_full_next)
        h_mask_next = self._advance_incident_h_3d(fields, masked_prev, dt_value)
        h_delta = self._compute_discrete_3d_h_phasor_delta(fields, dt=dt_value)
        h_reconstructed = {
        ...
        e_delta = self._compute_discrete_3d_e_phasor_delta(fields, dt=dt_value)
        e_reconstructed = {
    ...
    def _launch_power_normalization(self, fields, *, dt: float) -> float | None:
        ...
        if not bool(getattr(self, "_initialized", False)):
            ...
            if permittivity is None or resolution is None:
                return None
            self.initialize(permittivity, float(resolution), dt=float(dt))
        if not bool(getattr(self, "_is_3d", False)):
            return None

        shapes = tuple(
            tuple(getattr(fields, component).shape)
            for component in ("Ex", "Ey", "Ez", "Hx", "Hy", "Hz")
            if hasattr(fields, component)
        )
        signature = (
            id(fields),
            float(dt),
            shapes,
            float(getattr(self, "power", 1.0)),
            float(getattr(self, "wavelength", 0.0)),
            str(getattr(self, "direction", "")),
            str(getattr(self, "pol", "")),
        )
        cache = getattr(self, "_launch_power_normalization_cache", None)
        ...
        try:
            state = self._reconstructed_3d_launch_phasor_state(fields, dt=float(dt))
            field_profiles = self._deembed_3d_phasor_profiles(
                state,
                t_e=float(dt),
                t_h=0.5 * float(dt),
            )
            source_profiles, _indices = self._get_3d_profiles_and_indices()
            basis_profiles = {
                name: np.asarray(value, dtype=np.complex128)
                for name, value in source_profiles.items()
                if value is not None
            }
            axis = str(self._axis)
            d_area = float(self._resolution * self._resolution)
            forward, _backward = _make_3d_mode_basis_profiles(
                basis_profiles,
                axis=axis,
                d_area=d_area,
                direction_sign=float(self._direction_sign),
            )
            amplitude = _modal_overlap_3d_profiles(
                field_profiles,
                forward,
                axis=axis,
                d_area=d_area,
        ...
        emitted_power = float(abs(amplitude) ** 2)
        ratio = emitted_power / requested_power
        if (not np.isfinite(ratio)) or ratio <= 1e-24:
            return None

        self._launch_power_normalization_cache = (signature, ratio)
        return ratio
    ...
    @staticmethod
    def _normalize_3d_component_index(
        index: tuple,
        shape: tuple[int, int, int],
