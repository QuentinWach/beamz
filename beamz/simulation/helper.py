        ...
        if isinstance(source, ModeSource):
            ...
            for point in mode_profile:
                ...
                if fdtd.is_3d:
                    ...
                    # Hard source for 2D: Only inject Ez (most stable)
                    # H-fields are generated by Maxwell's equations from Ez
                    ez_amp = field_amplitudes.get('Ez', 0.0)
                    if ez_amp != 0.0 and np.isfinite(ez_amp):
                        if y < fdtd.Ez.shape[0] and x < fdtd.Ez.shape[1]:
