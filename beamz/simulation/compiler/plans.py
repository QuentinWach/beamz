...
def _build_monitor_plan(
    ...
    frequency_count = sum(1 for spec in monitor_specs if int(spec.freq_count) > 0)
    dft_count = sum(
        1 for spec in monitor_specs if bool(spec.dft_enabled and spec.freq_count > 0)
    )

    if dft_count and dft_count == len(monitor_specs):
        family = "dft_only"
    elif frequency_count and frequency_count == len(monitor_specs):
        family = "spectral_only"
    elif power_count and power_count == len(monitor_specs):
        family = "power_only"
    else:
        family = "mixed"
    ...
    for spec in monitor_specs:
        if spec.is_3d:
            regions.add((spec.min_dim0, spec.min_dim1, spec.normal_axis))
        else:
            regions.add((spec.record_interval, spec.freq_count, spec.normal_axis))
    geometry_family = "single_region" if len(regions) <= 1 else "mixed"
...
def _coefficient_layout_family(simulation, boundary_family: str) -> str:
    ...
    conductivity = np.asarray(simulation.fields.conductivity)
    permeability = np.asarray(simulation.fields.permeability)
    permittivity = np.asarray(simulation.fields.permittivity)
    if (
        conductivity.size
        and np.all(conductivity == conductivity.reshape(-1)[0])
        and np.all(permittivity == permittivity.reshape(-1)[0])
        and np.all(permeability == permeability.reshape(-1)[0])
    ):
