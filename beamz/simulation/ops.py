...
def curl_h_to_e_2d(h_fields, resolution, e_shapes, plane='xy'):
    ...
    if plane == 'xy':
        ...
        curl_ex = curl_ex.at[1:-1, 1:-1].set(
            (Hz[1:-1, 1:] - Hz[1:-1, :-1])/resolution - (Hy[1:, 1:-1] - Hy[:-1, 1:-1])/resolution
        )
...
def magnetic_conductivity_terms_2d_full(conductivity, permeability, hx_shape, hy_shape, hz_shape, plane):
    ...
    sigma_m_hx = jnp.reshape(sigma_m_hx, hx_shape) if sigma_m_hx.shape != hx_shape else sigma_m_hx
    sigma_m_hy = jnp.reshape(sigma_m_hy, hy_shape) if sigma_m_hy.shape != hy_shape else sigma_m_hy
    sigma_m_hz = jnp.reshape(sigma_m_hz, hz_shape) if sigma_m_hz.shape != hz_shape else sigma_m_hz
...
def magnetic_conductivity_terms_2d(conductivity, permeability, hx_shape, hy_shape):
    ...
    if conductivity.ndim < 2: return (jnp.zeros(hx_shape), jnp.zeros(hy_shape))  # No PML if conductivity is scalar
...
def magnetic_conductivity_terms_3d(conductivity, permeability, hx_shape, hy_shape, hz_shape):
    ...
    if conductivity.ndim < 3: return (jnp.zeros(hx_shape), jnp.zeros(hy_shape), jnp.zeros(hz_shape))
    # Slice arrays to match staggered Yee grid positions of each H-field component
    sigma_m_hx = jnp.reshape(conductivity[:-1, :-1, :] * permeability[:-1, :-1, :] * MU_0 / EPS_0, hx_shape)  # Hx at (z-1/2, y-1/2, x)
    sigma_m_hy = jnp.reshape(conductivity[:-1, :, :-1] * permeability[:-1, :, :-1] * MU_0 / EPS_0, hy_shape)  # Hy at (z-1/2, y, x-1/2)
    sigma_m_hz = jnp.reshape(conductivity[:, :-1, :-1] * permeability[:, :-1, :-1] * MU_0 / EPS_0, hz_shape)  # Hz at (z, y-1/2, x-1/2)
    return (sigma_m_hx, sigma_m_hy, sigma_m_hz)
