            ...
            if edge == 'left': # Start of horizontal axis (dim2)
                for i in range(dim2):
                    if coords2[i] < self.thickness:
                        ...
                        profiles[f'sigma_{axis2}'] = profiles[f'sigma_{axis2}'].at[:, i].set(self._sigma_profile(dist, self.thickness))
                        profiles[f'kappa_{axis2}'] = profiles[f'kappa_{axis2}'].at[:, i].set(self._kappa_profile(dist, self.thickness))
                        profiles[f'alpha_{axis2}'] = profiles[f'alpha_{axis2}'].at[:, i].set(self._alpha_profile(dist, self.thickness))
                ...
                for j in range(dim1):
                    if coords1[j] < self.thickness:
                        ...
                        profiles[f'sigma_{axis1}'] = profiles[f'sigma_{axis1}'].at[j, :].set(self._sigma_profile(dist, self.thickness))
                        profiles[f'kappa_{axis1}'] = profiles[f'kappa_{axis1}'].at[j, :].set(self._kappa_profile(dist, self.thickness))
                        profiles[f'alpha_{axis1}'] = profiles[f'alpha_{axis1}'].at[j, :].set(self._alpha_profile(dist, self.thickness))
            ...
            if edge == 'left':  # Start of x axis
                for i in range(nx):
                    if coords_x[i] < self.thickness:
                        ...
                        profiles['sigma_x'] = profiles['sigma_x'].at[:, :, i].set(self._sigma_profile(dist, self.thickness))
                        profiles['kappa_x'] = profiles['kappa_x'].at[:, :, i].set(self._kappa_profile(dist, self.thickness))
                        profiles['alpha_x'] = profiles['alpha_x'].at[:, :, i].set(self._alpha_profile(dist, self.thickness))
                ...
                for j in range(ny):
                    if coords_y[j] < self.thickness:
                        ...
                        profiles['sigma_y'] = profiles['sigma_y'].at[:, j, :].set(self._sigma_profile(dist, self.thickness))
                        profiles['kappa_y'] = profiles['kappa_y'].at[:, j, :].set(self._kappa_profile(dist, self.thickness))
                        profiles['alpha_y'] = profiles['alpha_y'].at[:, j, :].set(self._alpha_profile(dist, self.thickness))
                ...
                for k in range(nz):
                    if coords_z[k] < self.thickness:
                        ...
                        profiles['sigma_z'] = profiles['sigma_z'].at[k, :, :].set(self._sigma_profile(dist, self.thickness))
                        profiles['kappa_z'] = profiles['kappa_z'].at[k, :, :].set(self._kappa_profile(dist, self.thickness))
                        profiles['alpha_z'] = profiles['alpha_z'].at[k, :, :].set(self._alpha_profile(dist, self.thickness))
